...
class E131Module:
    ...
    def send_data(self, data: np.ndarray):
        ...
        for u in range(1, self.num_universes):
            self.sender[u].dmx_data =tuple(int(x) for x in flattened_data[start_index:start_index + 510])
            start_index += 510
        self.sender[self.num_universes].dmx_data = tuple(int(x) for x in flattened_data[start_index:])
